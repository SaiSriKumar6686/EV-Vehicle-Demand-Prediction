import streamlit as st #used for hosting the Model in Web
import pandas as pd #used to Process/Preprocess the DataSet
import numpy as np #used for Processing the Numbers
import joblib #used for storing the Best/Accurate Model locally
from datetime import datetime #used for converting the Non-Numerical Date & time to numerical date and time
import matplotlib.pyplot as plt #used for Plotting the Visualizations
...
st.write(f"📊 {county} — historical months available: {len(county_df)}, using last: {len(historical_ev)} values")
if len(historical_ev) < 3:
    st.info(f"Note: Only {len(historical_ev)} data point(s) available → using 0 for missing lags.")
...
for i in range(1, forecast_horizon + 1):
    ...
    # ── Safe lag extraction ──
    n = len(historical_ev)
    lag1 = historical_ev[-1] if n >= 1 else 0
    lag2 = historical_ev[-2] if n >= 2 else 0
    lag3 = historical_ev[-3] if n >= 3 else 0
    ...
    ev_growth_slope = np.polyfit(range(len(recent_cumulative)), recent_cumulative, 1)[0] \
        if len(recent_cumulative) >= 2 else 0
...
if historical_total > 0:
    ...
    st.warning("Historical EV total is zero → percentage change not computed.")
...
if multi_counties:
    ...
    for cty in multi_counties:
        ...
        st.write(f"Comparing {cty} — historical months: {len(cty_df)}, using last: {len(hist_ev)} values")
        if len(hist_ev) < 3:
            st.info(f"Note: Only {len(hist_ev)} data point(s) for {cty} → using 0 for missing lags.")
        ...
        for i in range(1, forecast_horizon + 1):
            ...
            # ── Safe lag extraction ──
            n = len(hist_ev)
            lag1 = hist_ev[-1] if n >= 1 else 0
            lag2 = hist_ev[-2] if n >= 2 else 0
            lag3 = hist_ev[-3] if n >= 3 else 0
            ...
            recent_cum = cum_ev[-6:]
            ev_slope = np.polyfit(range(len(recent_cum)), recent_cum, 1)[0] \
                if len(recent_cum) >= 2 else 0
    ...
    for cty in multi_counties:
        ...
        # Last historical point = total rows - forecast_horizon
...
st.markdown("Prepared for the **Internship by SaiSriKumar Parimi**")